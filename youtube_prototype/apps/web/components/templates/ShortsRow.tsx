...
function ShortCard({ title, thumbnail, views }: { title: string; thumbnail: string; views: number }) {
    ...
    <section className="px-6 py-3">
      ...
      <div className="-mx-6 overflow-x-auto px-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="flex gap-3">
          {shorts.map((s) => (
            <ShortCard key={s.id} title={s.title} thumbnail={s.thumbnail} views={s.views} />
          ))}
        </div>
      </div>
    </section>
